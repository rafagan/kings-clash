...
public class UdpLobbyServer : LobbyServer
{
	...
	public override bool Start (int listenPort)
	{
        ...
        Debug.Log("UDP Lobby Server started on port " + listenPort);
	...
	bool ProcessPacket (Buffer buffer, IPEndPoint ip)
	{
		...
		switch (request)
		{
			case Packet.RequestAddServer:
				...
				mList.Add(ent, mTime);
                Debug.LogWarning(ip + " adicionou um novo servidor (" + ent.internalAddress + ", " + ent.externalAddress + ")");
				return true;
			}
			case Packet.RequestRemoveServer:
			{
                ...
                Debug.LogWarning(ip + " removed a server (" + internalAddress + ", " + externalAddress + ")");
				return true;
			}
    ...
    public override void AddServer(string name, string password, string host, int playerLimit, bool joinInGame, int playerCount, IPEndPoint internalAddress, IPEndPoint externalAddress)
    {
		mList.Add(name, password, host, playerLimit, joinInGame, playerCount, internalAddress, externalAddress, mTime);
	}

...
/// <summary>
/// This example script shows how to create a chat window powered by the Network framework.
...
public class ChatManager : NetBehaviour
{
	...
	NetList<ChatEntry> mChatEntries = new NetList<ChatEntry>();
	...
	{
		mName = NetManager.playerName;

		string text = "Players online: ";
		NetList<Player> players = NetManager.players;
		
		for (int i = 0; i < players.size; ++i)
		{
			if (i > 0) text += ", ";
			text += players[i].name;
			if (players[i].id == NetManager.playerID) text += " (you)";
		}
	...
	{
		AddToChat(p.name + " joined.", Color.black);
	}
	...
	{
		AddToChat(p.name + " left.", Color.black);
	}
	...
	{
		AddToChat(previous + " changed name to " + p.name, Color.black);
	}
	...
	[RFC] void OnChat (int playerID, string text)
	{
		...
		Color color = (player.id == NetManager.playerID) ? Color.green : Color.white;
	...
	void Send ()
	{
		...
		{
            Package.Send("OnChat", Target.All, NetManager.playerID, mInput);
			mInput = "";
		}
	}
	...
	void OnGUI ()
	{
		...
		if (GUI.Button(new Rect(cx + 60f, cy - 170f, 80f, 24f), "Change"))
		{
			...
			NetManager.playerName = mName;
		}

		GUI.SetNextControlName("Chat");
		mRect = new Rect(cx - 200f, cy - 120f, 400f, 300f);
		GUI.Window(0, mRect, OnGUIWindow, "Chat");

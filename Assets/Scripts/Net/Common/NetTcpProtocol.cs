...
namespace Net
{
    ...
    public class TcpProtocol : Player
    {
        ...
        NetList<Socket> mConnecting = new NetList<Socket>();
        ...
        public void Connect(IPEndPoint externalIP, IPEndPoint internalIP)
        {
            ...
            // Alguns roteadores, como o Asus RT-N66U não suportam NAT Loopback, e conectar em um IP externo
            // vai na verdade conectar o roteador. Então se é um IP local, conecta nele primeiro.
        ...
        /// <summary>
        /// Tenta estabeler conexão com o tcpEndPoint atual.
        /// </summary>
        ...
        bool ConnectToTcpEndPoint()
        {
            ...
            else Error("Não foi possibel conectar no endereço especificado.");
            return false;
        }
        ...
        public bool VerifyResponseID(Packet packet, BinaryReader reader)
        {
            if (packet == Packet.ResponseID)
            {
                ...
                {
                    id = 0;
                    Error("Versoes incompativeis! Servidor esta rodando a versao " + serverVersion + " enquanto voce tem a versao " + version);
                    Close(false);
                }
            }
#if UNITY_EDITOR

		...
		//if (AssetDatabase.GetAssetPath( (_target.gameObject.GetComponent(typeof(MeshRenderer)) as MeshRenderer).sharedMaterial)!="") {
			...
			if (_target.blendedObject) {
				...
				if (GUILayout.Button(new GUIContent("Stick to Blended Object along normals"))) {
					...
					_target.renderer.castShadows=false;
					EditorUtility.SetDirty(target);
				}
				...
				if (_target.BuildMeshFlag) {
					...
					if (terrainComp) {
						...
						if (geomScript.controlA!=null) {
							float cellW=_target.blendedObject.renderer.bounds.size.x/(geomScript.controlA.width)*Mathf.Pow(2.0f, -1.0f*_target.addTrisSubdivision);
							float cellH=_target.blendedObject.renderer.bounds.size.z/(geomScript.controlA.height)*Mathf.Pow(2.0f, -1.0f*_target.addTrisSubdivision);
							int _w=Mathf.RoundToInt((_target.paint_size*2)/cellW+1);
							int _h=Mathf.RoundToInt((_target.paint_size*2)/cellH+1);
							_w=_w>44 ? 44:_w;
							_h=_h>44 ? 44:_h;
							...
							for(int i=0; i<_w; i++) {
								for(int j=0; j<_h; j++) {
									...
									RaycastHit hit;
									if (terrainCollider.Raycast(new Ray(new Vector3(cover_verts[cover_num].x, _target.blendedObject.renderer.bounds.max.y, cover_verts[cover_num].z), Vector3.down), out hit, Mathf.Infinity)) {
